...
import { getErrorMessage, cocoaTimestampToDate, formatDate, hoursAgoToCocoaTimestamp, daysAgoToCocoaTimestamp, formatPhoneNumber, truncate, } from "./utils.js";
// Database path
 ...
 * Get recent messages
 */
export function getRecentMessages(hours = 24, contact, limit = 50) {
    const db = getDatabase();
    try {
 ...
 */
export function getConversation(contact, days = 7, limit = 100) {
    const db = getDatabase();
    try {
      ...
      LIMIT ?
    `)
            .all(cocoaTimestamp, `%${contact}%`, formattedContact, `%${formattedContact}%`, limit);
        return messages.map((msg) => ({
            id: msg.rowid,
            text: msg.text || "(attachment or empty)",
            date: formatDate(cocoaTimestampToDate(msg.date)),
 ...
 */
export function searchMessages(query, days = 30, limit = 50) {
    const db = getDatabase();
    try {
        const cocoaTimestamp = daysAgoToCocoaTimestamp(days);
        const messages = db
      ...
      LIMIT ?
    `)
            .all(cocoaTimestamp, `%${query}%`, limit);
        return messages.map((msg) => ({
            id: msg.rowid,
            text: msg.text || "",
            date: formatDate(cocoaTimestampToDate(msg.date)),
 ...
 * List all chats
 */
export function listChats(limit = 30, includeGroupChats = true) {
    const db = getDatabase();
    try {
        let query = `
      SELECT
 ...
 */
export function getAttachmentsForMessage(messageId) {
    const db = getDatabase();
    try {
 ...
 */
export function getRecentAttachments(days = 7, contact, limit = 50) {
    const db = getDatabase();
    try {
        const cocoaTimestamp = daysAgoToCocoaTimestamp(days);
        let query = `
 ...
 */
export function getGroupChatMembers(chatIdentifier) {
    const db = getDatabase();
    try {
        const members = db
            .prepare(`
 ...
 */
export function findChatByContact(contact) {
    const db = getDatabase();
    try {
        const formattedContact = formatPhoneNumber(contact);
        const chat = db
      ...
      LIMIT 1
    `)
            .get(`%${contact}%`, formattedContact, `%${formattedContact}%`);
        if (!chat)
            return null;
        return {
            id: chat.rowid,
